...
export default class GoTrueClient {
  ...
  async getSessionFromUrl(options?: {
    ...
    try {
      ...
      const provider_token = getParameterByName('provider_token', url || undefined)
      const access_token = getParameterByName('access_token', url || undefined)
      if (!access_token) throw new Error('No access_token detected.')
      const expires_in = getParameterByName('expires_in', url || undefined)
      if (!expires_in) throw new Error('No expires_in detected.')
      const refresh_token = getParameterByName('refresh_token', url || undefined)
      if (!refresh_token) throw new Error('No refresh_token detected.')
      const token_type = getParameterByName('token_type', url || undefined)
